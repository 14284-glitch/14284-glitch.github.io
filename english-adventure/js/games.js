const AdventureGames=(()=>{
  ...
  function order(item){
    ...
    return `<div class="game-stage"><p class="eyebrow">模式三・句子排序</p><div class="scene">${item.image}</div><h3>${item.translation}</h3><p>依序點選單字，排成正確句子。</p><div class="answer-bank" id="answerBank"></div><div class="word-bank" id="wordBank">${words.map(x=>`<button class="word-chip" data-word="${x.word}">${x.word}</button>`).join("")}</div><button class="primary" id="checkOrder">完成句子</button><div class="feedback" id="gameFeedback" aria-live="polite"></div></div>`;
  }
